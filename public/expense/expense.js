...
async function submitExpense(e) {
    try {
        ...
        let responseData = await axios.post(`http://16.16.208.254:3000/expense/add-expense?itemsPerPage=${itemsPerPage}`, expenseDetails, { headers: { 'Authorization': token } });
...
async function removefromscreen(data) {
    try {
        ...
        let itemsPerPage = localStorage.getItem('itemsPerPage');
        let responseData = await axios.delete(`http://16.16.208.254:3000/expense/delete-expense/${data.id}?page=${page}&itemsPerPage=${itemsPerPage}`, { headers: { 'Authorization': token } });
...
document.getElementById('rzp-button1').onclick = async function (e) {
    let response = await axios.get('http://16.16.208.254:3000/purchase/purchasemembership', { headers: { 'Authorization': token } });
    var options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id,
        "handler": async function (response) {
            let respData = await axios.post('http://16.16.208.254:3000/purchase/updatetransactionstatus', {
    ...
    rzp1.on('payment.failed', async function (response) {
        await axios.post('http://16.16.208.254:3000/purchase/failedtransactionstatus', {
            order_id: options.order_id,
        }, { headers: { 'Authorization': token } });
...
function leaderBoardButton() {
    ...
    document.getElementById('showLeaderboardId').onclick = async function (e) {
        try {
            e.preventDefault();
            let responseData = await axios.get('http://16.16.208.254:3000/premium/showLeaderBoard', { headers: { 'Authorization': token } });
            leaderboardHeading();
...
function downloadButtonFun() {
    ...
    document.getElementById('downloadFile').onclick = async function (e) {
        try {
            e.preventDefault();
            let respData = await axios.get('http://16.16.208.254:3000/expense/download', { headers: { "Authorization": token } });
...
function ListsOfDownloadButtonFunc() {
    ...
    document.getElementById('listOfUrls').onclick = async function (e) {
        try {
            e.preventDefault();
            let respData = await axios.get('http://16.16.208.254:3000/expense/lists', { headers: { "Authorization": token } });
            if (respData.status === 201) {
...
async function getProducts(page) {
    try {
        let itemsPerPage = localStorage.getItem('itemsPerPage');
        let responseData = await axios.get(`http://16.16.208.254:3000/expense/get-expense?page=${page}&itemsPerPage=${itemsPerPage}`, { headers: { 'Authorization': token } });
        if (responseData.status === 201) {
